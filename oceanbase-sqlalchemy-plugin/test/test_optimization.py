...
import os
import time
from sqlalchemy import create_engine, inspect, text, MetaData, Table, Column, Integer, String, Date, CheckConstraint, Numeric
...
class TestOceanBaseOptimization:
    ...
    def test_actual_connection_optimization(self):
        ...
        if not all([host, port, username, password, service_name]):
            pytest.skip("Skipping actual connection test: environment variables not configured")
        
        try:
            # Use secure connection string building method
            from oceanbase_sqlalchemy.utils import build_safe_connection_string
            connection_string = build_safe_connection_string(username, password, host, port, service_name)
            ...
            with engine.connect() as conn:
                ...
                import uuid
                test_table_name = f"TEST_OPTIMIZATION_TABLE_{uuid.uuid4().hex[:8].upper()}"
                metadata = MetaData()
                ...
                print(f"🔨 Using SQLAlchemy API to create test table: {test_table_name}")
                ...
                table_found = any(table.upper() == test_table_name.upper() for table in tables)
                assert table_found, f"Test table {test_table_name} not found in table list"
                ...
                assert 'id' in [col.lower() for col in pk_info['constrained_columns']], "Primary key constraint incorrect"
                
                # Get foreign key constraints
                fk_info = inspector.get_foreign_keys(test_table_name)
                print(f"Foreign key info: {fk_info}")
                assert fk_info == [], "Newly created table should not have foreign key constraints"
                ...
                assert len(unique_info) == 0, "Simplified table should not have unique constraints"
                ...
                assert len(check_info) == 0, "Simplified table should not have check constraints"
                
                print("✅ Actual connection test successful, all constraint queries used optimization methods")
                print("Note: Please check the SQL logs above to confirm whether optimized SQL statements were used")
